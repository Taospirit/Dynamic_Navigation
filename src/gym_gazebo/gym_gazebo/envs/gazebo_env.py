...
class GazeboEnv():
    ...
    def check_obs_goal(self, arrived_dist, s, g):
        obs_num = len([obs for obs in s if -10 < obs[0] < 10])
        done = True
        if obs_num == 0:
            done = False
    ...
    #region: for gym
    def agent_step(self, cmd):
        cmd_vel = Twist()
        cmd_vel.linear.x = cmd[0]
        cmd_vel.angular.z = cmd[1]
        self.pub_agent.publish(cmd_vel)

    def obs_step(self, planner, obs_start_list, obs_goal_list):
        obs_num = len([obs for obs in obs_start_list if -10 < obs[0] < 10])
        for i in range(obs_num):
            cmd_vel = Twist()
            self_pose = self.gazebo_obs_states[i]
            goal_pose = {'x':obs_goal_list[i][0], 'y':obs_goal_list[i][1]}
            ...
            planner.init_env(self_pose, goal_pose, obs_pose_list)
            # parameter to be fixed
            planner.set_katt(1.0)
            planner.set_krep(50)
            planner.set_linear_max(2.0)
            planner.set_linear_min(0.3)
            planner.set_angluar_max(2.0)
            planner.set_angluar_min(0.5)
            planner.set_safe_dist(3.0)
            planner.set_arrived_dist(self.dist_near_goal)
        #==== robot tf
            # cmd_vel.linear.x, cmd_vel.angular.z = planner.get_cmd()
    ...
    def gazebo_pub_states(self, gazebo_state_name, pose_list, index_list=None):
        theta_list = [pose[2] for pose in pose_list]
        quat_list = [self.quaternion_from_euler(np.radians(theta)) for theta in theta_list]
        model_state = ModelState()
    ...
    #         # self.planner.set_angluar_min(0.5)
