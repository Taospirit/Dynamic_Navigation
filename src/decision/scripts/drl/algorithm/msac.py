...
from drl.utils import ReplayBuffer, PriorityReplayBuffer
...
class MSAC(BasePolicy):
    def __init__(
        ...
        self.actor_eval_optim = optim.Adam(self.actor_eval.parameters(), lr=actor_lr)
        self.critic_eval_optim = optim.Adam(self.critic_eval.parameters(), lr=critic_lr)
    ...
    def learn_critic_dist(self, obs, act, rew, next_obs, mask):
        with torch.no_grad():
            ...
            p1_next, p2_next = self.critic_target.get_probs(next_obs, next_act) # [batch_size, num_atoms]
            p_next = torch.stack([torch.where(z1.sum() < z2.sum(), z1, z2) for z1, z2 in zip(p1_next, p2_next)])
    ...
    def learn_critic(self, obs, act, rew, next_obs, mask):
        with torch.no_grad():
            ...
            if self.use_priority:
                q_target = rew + mask * (self._gamma ** self.n_step) * q_next.cpu()
    ...
    def learn_actor_dist(self, obs):
        ...
        p_next = torch.stack([torch.where(p1.sum() < p2.sum(), p1, p2) for p1, p2 in zip(p1_next, p2_next)])
    ...
    def learn(self):
        ...
        for _ in range(self._update_iteration):
            if self.use_priority:
                S, A, R, S_, M, indices, weights = self.buffer.sample(self._batch_size)
